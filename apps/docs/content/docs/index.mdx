...
Initially, we maintain a separate TypeScript library called `rehooks-ts`, which houses all our custom hooks. These hooks are then converted into a JSON file, effectively serving as an [API](/docs/api#limitations) for the `rehooks-cli`. In essence, when you need a hook, you simply choose from the curated list available through the [API](/docs/api#limitations), and the CLI handles the import directly into your project with ease. This streamlined process ensures seamless integration and rapid development.
